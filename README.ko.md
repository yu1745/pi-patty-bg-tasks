...
| 상수 | 값 | 목적 |
|---|---:|---|
| 폴링 간격 | 30초 | `/proc`를 계속 훑지 않고 로그/프로세스 관측 갱신 |
| 최소 잡 나이 | 60초 | 정상 시작 지연을 분류하지 않음 |
| 로그 정적 게이트 | 60초 | 반복 출력이 아니면 1분간 로그 증가가 없어야 자동 평가 |
| 반복 출력 게이트 | 120초 | 정규화된 반복 꼬리가 2분간 지속되어야 함 |
| Jev 재검사 | 60초 | API 사용을 제한하고 새 샘플 확보 |
| 연속 고신뢰 | 2회 | 자동 알림은 2회 연속, 수동 검사는 1회 |
| 알림 쿨다운 | 15분 | 같은 잡의 반복 알림 방지 |
| 전송 로그 꼬리 | 8KB | 증거는 유지하되 비용·노출은 제한 |
| 임계값 | 차단 증거 ≥ .70; 진행/서비스/유한 대기 각각 ≤ .60 | 알림용으로 보정; 자동 알림은 여전히 2회 연속 판정 필요 |
| Jev 데드라인 | 20초 | 모델/API 지연이 새 정체가 되지 않게 함 |
